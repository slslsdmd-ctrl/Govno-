...
logging.basicConfig(
    ...
    level=logging.INFO,
    handlers=[
        logging.FileHandler("sombet.log"),
...
# ========== КОНФИГУРАЦИЯ ==========
BOT_TOKEN = "8601391472:AAGyfWUcbwh-UnphfXLep6LaSBqHzwRjTFo"
ADMIN_ID = 8039111975
SUPPORT = "@sombetsupport"
CASINO_NAME = "SOM BET"
...
users = {}
withdraw_requests = {}
DATA_FILE = "sombet_data.json"
REQUESTS_FILE = "sombet_requests.json"
...
async def demo_mode_menu(update, context):
...
async def demo_dice_over(update, context):
    uid = str(update.effective_user.id)
    await ensure_user_exists(uid)
    if len(context.args) != 1:
...
async def demo_dice_even(update, context):
    uid = str(update.effective_user.id)
    await ensure_user_exists(uid)
    if len(context.args) != 1:
...
async def game_coin(update, context):
    query = update.callback_query
    await query.answer()
...
async def dice_sum(update, context):
    uid = str(update.effective_user.id)
    await ensure_user_exists(uid)
    if len(context.args) != 2:
...
async def dice_over(update, context):
    uid = str(update.effective_user.id)
    await ensure_user_exists(uid)
    if len(context.args) != 2:
...
async def dice_even(update, context):
    uid = str(update.effective_user.id)
    await ensure_user_exists(uid)
    if len(context.args) != 2:
...
async def add_deposit(update, context):
...
async def balance(update, context):
    ...
    uid = str(update.effective_user.id)
    await ensure_user_exists(uid)
    await message.reply_text(f"💰 БАЛАНС: {round_ton(users[uid]['balance'])} TON", reply_markup=main_menu())
...
async def support(update, context):
    if update.callback_query:
        ...
        message = update.message
    await message.reply_text(f"📞 ПОДДЕРЖКА: {SUPPORT}", reply_markup=main_menu())
...
async def callback_handler(update, context):
    ...
    data = query.data
    uid = str(update.effective_user.id)
    logger.info(f"Callback: user={uid}, data={data}")
...
async def main():
    ...
    logger.info(f"📝 ЛОГИ СОХРАНЯЮТСЯ В sombet.log")
